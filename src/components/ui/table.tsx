...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b border-border/70 transition-colors duration-150 hover:bg-accent/55 data-[state=selected]:bg-primary/[0.06]",
        className
      )}
      {...props}
    />
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "eyebrow h-10 whitespace-nowrap px-4 text-left align-middle text-[10px] text-muted-foreground",
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "whitespace-nowrap px-4 py-3 align-middle first:pl-5 last:pr-5 md:first:pl-6 md:last:pr-6",
        "[&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]",
        className
      )}
      {...props}
